...
-- Approach: Since there are same entries for a date for a customer, I'm choosing to find unique days on which customer visited.
...
-- Approach: Since the question is only to fetch first item, I choose to assign numbers to the items orderered in menu by customer
-- and fetch the first one from list.
...
-- Approach: Firstly fetching the count of items ordered by customer and then assigning rank to them in descending order 
-- then fetching the item ranking as 1
...
-- I accomplished this using CTE and assigning dense_rank to the item ordered in descending order of order date which helped 
...
-- I fetched the 
select c.customer_id, count(s.product_id) , sum(m.price)
from members c
join sales s on c.customer_id = s.customer_id
join menu m on m.product_id = s.product_id
where s.order_date < c.join_date 
...
-- Approach: In this I summed up the points based on item name and grouped using customer id to fetch results
...
-- Approach: In this firstly, I found the the first week end date for both users after they become member and then put the condition of 
...
-- Second Bonus question: To rank the customer purchases for members only and Null rank for non_members
